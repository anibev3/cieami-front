...
interface ReparateursDataTableProps {
  data: Reparateur[]
  loading?: boolean
}

export function ReparateursDataTable({
  data,
  loading = false,
}: ReparateursDataTableProps) {
  const { setOpen, setCurrentRow } = useReparateurs()
  const [rowSelection, setRowSelection] = React.useState({})
  const [columnVisibility, setColumnVisibility] = React.useState<VisibilityState>({})
  const [columnFilters, setColumnFilters] = React.useState<ColumnFiltersState>([])
  const [sorting, setSorting] = React.useState<SortingState>([])

  const handleView = (reparateur: Reparateur) => {
    setCurrentRow(reparateur)
    setOpen('view')
  }

  const handleEdit = (reparateur: Reparateur) => {
    setCurrentRow(reparateur)
    setOpen('update')
  }

  const handleDelete = (reparateur: Reparateur) => {
    setCurrentRow(reparateur)
    setOpen('delete')
  }

  const columns: ColumnDef<Reparateur>[] = React.useMemo(
    () => [
      {
        accessorKey: 'code',
        header: 'Code',
      ...
      },
      {
        ...
        header: 'Actions',
        cell: ({ row }) => {
          const reparateur = row.original
          return (
            <div className="flex items-center space-x-2">
              <button
                onClick={() => handleView(reparateur)}
                className="text-blue-600 hover:text-blue-800 text-sm font-medium"
              >
                Voir
              </button>
              <button
                onClick={() => handleEdit(reparateur)}
                className="text-green-600 hover:text-green-800 text-sm font-medium"
              >
                Éditer
              </button>
              <button
                onClick={() => handleDelete(reparateur)}
        ...
        <div className="rounded-md border">
          <div className="h-96 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="mt-2 text-sm text-gray-600">Chargement des réparateurs...</p>
            </div>
          </div>
        </div>
      </div>
