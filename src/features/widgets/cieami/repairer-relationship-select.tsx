...
import {  ChevronsUpDown, Check, Loader2, X, Wrench } from 'lucide-react'
...
interface RepairerRelationshipSelectProps {
...
export function RepairerRelationshipSelect({
  value,
  onValueChange,
...
}: RepairerRelationshipSelectProps) {
  ...
  const fetchRelationships = useCallback(async (page = 1) => {
    setLoading(true)
    try {
      const response = await repairerRelationshipService.list(page)
